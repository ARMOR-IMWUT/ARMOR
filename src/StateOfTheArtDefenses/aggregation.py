...
from src.models.models import CNNFashion_Mnist, CNNMnist
...
def list_todict(weight_list, args, useGAN=False, label=0):
    if args.dataset == "mnist":
        if useGAN:
            shape = CNNMnist(args, True, label)
        else:
            shape = CNNMnist(args)
    else:

        if (useGAN):
            shape = CNNFashion_Mnist(args, True, label)
        else:
            shape = CNNFashion_Mnist(args)
