...
class GAN_ARMOR(object):
    ...
    def train_generator(self, args, label, epoch):
        ...
        diff_model = (1 / (args.ganepochs)) * (torch.Tensor(orderdict_tolist_adapt(self.new_model.state_dict(),
                                                                                   args.dataset == "fmnist")))  # - torch.Tensor(orderdict_tolist_adapt(self.old_model.state_dict(), args.dataset=="fmnist")))
        ...
        for epch in range(args.ganepochs):
            ...
            loss_inspected_, accuracy_inspected_, average_output_inspected_ = test(args, self.new_model, device,
                                                                                   out_fake, label)
    ...
    def train_generator_modif(self, args, label, epoch):
        ...
        diff_model = (1 / (args.ganepochs)) * (torch.Tensor(
            orderdict_tolist_adapt(self.new_model.state_dict(), args.dataset == "fmnist")) - torch.Tensor(
            orderdict_tolist_adapt(self.old_model.state_dict(), args.dataset == "fmnist")))
    ...
    def poisoningDetection(self, epoch):
        ...
        import time
        diff_loss, loss_inspected, loss_clean, loss_clean2, diff_accuracy, accuracy_clean, accuracy_inspected, average_output_clean, average_output_clean2, average_output_inspected, ratio1_, ratio2_ = (
        [] for i in range(12))
        # starting time
        start = time.time()
        ...
        for label in range(0, args.num_classes):
            ...
            loss_inspected_, accuracy_inspected_, average_output_inspected_ = test(args, model_poisoned, device,
                                                                                   fake_poisoned, label)
            loss_clean_, accuracy_clean_, average_output_clean_ = test(args, model_good_1, device, fake_poisoned, label)
            loss_clean2_, accuracy_clean2_, average_output_clean2_ = test(args, model_good_2, device, fake_poisoned,
                                                                          label)
            ...
            if (loss_inspected_ != 0 and loss_clean_ != 0 and loss_clean2_ != 0):
                ratio1 = np.abs(loss_inspected_ - loss_clean_) / max(loss_inspected_,
                                                                     loss_clean_)  # loss_clean_/ (loss_inspected_ + 10**(-15)) <
                ratio2 = np.abs(loss_inspected_ - loss_clean2_) / max(loss_inspected_,
                                                                      loss_clean2_)  # loss_clean2_/ (loss_inspected_ + 10**(-15))<
